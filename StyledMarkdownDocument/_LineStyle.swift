...
extension Node {
     ...
     */
    func apply<T: Parent>(style: T.Type, in document: StyledMarkdownDocument) -> Void {
...
extension Text {
     ...
     */
    func apply<T: Parent>(style: T.Type, to range: NSRange, in document: StyledMarkdownDocument) -> Void {
        guard has(style: style) == false else {
            /// Style is already applied, no need to continue.
            return
        }
